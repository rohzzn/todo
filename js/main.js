...
$(document).on('ready', function () {
        ...
        mainPage.fullpage({
          ...
          onLeave: function (index, nextIndex, direction) {
            ...
            scrollIndicator.width(
              (nextIndex / pageSectionDivs.length) * 100 + '%',
            )
          ...
          afterLoad: function (anchorLink, index) {
            if (index == 1) {
              scrollIndicator.width(
                (index / pageSectionDivs.length) * 100 + '%',
              )
            }
  ...
  $('#about-accordion .card').on('show.bs.collapse', function (e) {
    const imgUrl = $(e.currentTarget).data('image')
    $(e.currentTarget)
      .parents('.section-content')
      .get(0)
      .style.setProperty('--bg-url', `url(../img/${imgUrl})`)
  })
})

document.getElementById('loading-video').addEventListener(
  'ended',
  function () {
